...
//
// V-GENERIC. Every kernel here takes its lane type as a parameter and touches it
// only through +, -, * and construction from T, so the same text is the SIMD
// kernel at V = xsimd::batch<T> and the plain scalar kernel at V = T. The long
// double backend (scalar_fft.hpp) instantiates the scalar form; nothing in the
// tree carries a second copy of the radix math, and reading these kernels at
// V = T is the readable statement of what the vector passes compute.
// ============================================================================
...
// contracts to FMAs). V-generic: batch<T> and scalar tail. The constants fold at
