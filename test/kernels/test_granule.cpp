...
// (one lane-flipped radix-3 pool entry) and must fail, ctest WILL_FAIL side.
...
// The admission truth table, evaluated over the whole generated catalog: the predicate
// answers true exactly at the measured set, so a catalog change or a flipped bit is loud
// at compile time in both precisions. ISA-probe-relative, so it holds in every build.
namespace {
...
constexpr bool granule_admit_table_ok(std::index_sequence<Is...>) {
    return ((granule_rows_admit_v<static_cast<unsigned>(CODELET_CATALOG_SIZES[Is]), T> ==
             (kGranuleFmaddsub && (CODELET_CATALOG_SIZES[Is] == 12 ||
                                   CODELET_CATALOG_SIZES[Is] == 24 ||
                                   (CODELET_CATALOG_SIZES[Is] == 16 &&
...
// built-but-unadmitted 9..15 OFF (they compile and pass above; the bit waits on the
// per-N sweep). ISA-probe-relative positives, so the table holds in every build flavor.
...
static_assert(!granule_rows_admit_v<13, double>);
static_assert(!granule_rows_admit_v<15, float>);
...
TEST_CASE("granule catalog admission table is exact, and nonempty at the native build",
    ...
    // nothing is a bug shaped here), and at v2 the family is if constexpr-dead.
