...
//   out of place. Coverage: a generic masked tail (465's `ido==5 < W8`) and a
//   ragged one (25575's `ido==341 % 8 = 5`).
// * Elected-route cells: route name plus impulse flatness at 25575 (DP elects
//   5-15-31-11) and at 262143 / 1048575. The O(N^2) reference is infeasible at
//   262143 / 1048575, and `X[k] == 1` for impulse input is exact at any N.
// * Boundary trap: bare or bookended generic fallback chains (13, 26, 65, 806) must
//   stay out of `iterative_dif`. Electing one throws at dispatch (poet `no_match`).
...
// API pin: threading is fixed at plan creation. `exec_options` carries no pool
// member, so no call site can hold one (compile-fail-by-construction).
...
// Driver-level force: the same kernels and the same tape, without the `plan_impl`
// availability veto. The veto keeps a chiplet prime at a SHORT `ido` out of
// production routes; see `dif_chain_supported`. The chiplet cells below sit on the
// vetoed side by construction (97/83 at `ido` 8..44).
template<typename T>
...
TEMPLATE_TEST_CASE("generic prime pass: forced chains vs naive DFT", "[accuracy][generic]",
    ...
    // Generic strictly mid-chain; every pass radix is valid on ALL ISAs. The
    // register-parametric radix set drops 9/15/16/25/32 when regs <= 16, and forcing
    // a dropped radix throws by design. Masked tails: 465's `ido=5` masks at W=2/4
    // and is full width on no ISA, and 25575's generic pass sees `ido=11`.
    for (bool fwd : {false, true}) {
        ...
        // Generic at a W-multiple `ido`: the `es2` W-blocked SoA generation can
        // bracket the pass. 832 = 8*13*8, and the `ido` at the 13 is 8.
        check_forced<T>(832, dif_factor_plan{8, 13, 8}, fwd, false);
        check_forced<T>(832, dif_factor_plan{8, 13, 8}, fwd, true);
    }
    // 25575 round-trips through a chain one slot deeper in the generic region, both
    // directions, in place and out, with ISA-portable pass radices.
...
TEMPLATE_TEST_CASE("prime chiplet census: every pool prime vs naive DFT", "[accuracy][generic]",
                   float, double) {
    using T = TestType;
    // Every pool prime's chiplet table, forced. Bookends are ISA-portable. The `ido`
    // at the generic pass: 4 (masked at W>=8-f64), 8 (full tiles at W<=8), 7 (ragged).
...
TEMPLATE_TEST_CASE("prime chiplet pass: forced chains vs naive DFT", "[accuracy][generic]",
    ...
    // `dif_generic_radices`; all bookend radices below are valid on all ISAs. Masked
    // tails: 1005's `ido==5` at the 67, 4268's `ido==11` (masks at every W),
    // 25564's `ido==44` at the 83 (ragged at every W). Every prime below exceeds
    // `kGenericStarvedTailMinRadix` at a part-width `ido`, so every such prime takes
    // the raw driver path. The plan path keeps the `plan_impl` availability veto;
    // the plan path's generic coverage is 465 / 3135 / 832 / 25575.
    for (bool fwd : {false, true}) {
    ...
    // Round-trip force: O(N^2) references are suite-hostile past ~5k on the baseline
    // ISAs. The elected-route impulse flats below and the 1005/4268 reference checks
    // above still pin a dropped sign in the chiplet's consteval tables.
    // 6208 = 8.97.8 sits the 97 at `ido` 8, a W-multiple on f64, so the `es2`
    // W-blocked bracket sees full tiles. 25564 = 7.83.11.4 puts the 83 mid-chain at
    // `ido` 44, a ragged tail at every W.
    for (bool ip : {false, true}) {
...
TEMPLATE_TEST_CASE("elected generic-pass sizes: route + impulse flatness", "[accuracy][generic]",
    ...
    using T = TestType;
    {
        ...
        // f64 elects `iterative_dif` at 25575. The winning chain and the
        // `iterative_dif`-vs-`bluestein` outcome are ISA-dependent, so pin accuracy,
        // not routes.
    ...
    // Chiplet cells: on f64 W=8 the candidate walk elects a 97-carrying chain for
    // these sizes. Other ISAs route elsewhere; impulse flatness pins accuracy on any
    // ISA.
    impulse_flat<T>(194000, 4.0);
    impulse_flat<T>(122220, 4.0);
    // A broken election silently reverts these sizes to `bluestein`. Wherever the
    // candidate walk finds a runnable chain for a (T, ISA) pair, the route must stay
    // `iterative_dif`.
...
TEMPLATE_TEST_CASE("boundary-generic chains stay out of iterative_dif", "[coverage][generic]",
    ...
    for (const std::size_t N :
        ...
        // The size still transforms correctly wherever the plan routes.
    ...
    // `dif_pass_last<31>`, so executing the chain must throw (poet `no_match`)
    // instead of leaving a silently untransformed buffer.
    {
    ...
    // Veto shape: the plan admits a big prime at `ido` >= W. The O(p^2) loss behind
    // the `ido<64` veto is gone with the chiplet. `ido < W` stays out, because
    // half-idle tiles still lose to `bluestein`. The DP also prices the `ido < W`
    // shape out at 2x the valley tier, so the DP elects only supported placements:
    // 9603's 97 lands at `ido=33` at W=16, while 776's only placements `{2,4}` stay
    // out for W>4.
    {
