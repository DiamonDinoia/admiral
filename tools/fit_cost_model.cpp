// fit_cost_model — regenerate include/admiral/detail/base_cost_model.hpp from
...
// f64 W=8 and f32 W=8 are both real targets.
std::vector<double> feat_own(const std::string& form, std::size_t n, std::size_t w,
    ...
    if (form == "codelet") {
        ...
        // function of (n, lpf, nfac) expresses that — lpf(n) == n for every prime —
        ...
        // rader.hpp -- length-(p-1) CYCLIC convolution, no chirp-z zero padding, so the
...
void emit(std::ostream& os, const std::vector<std::string>& present,
    ...
    os << "#pragma once\n"
          ...
          "// (radix content, lane waste, padding, footprint), so a target this tool has\n"
          "// never swept routes on them too, and nothing here is keyed to a machine. One\n"
