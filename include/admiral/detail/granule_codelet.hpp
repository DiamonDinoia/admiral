...
template<unsigned N, typename T>
inline constexpr bool granule_cols_admit_v = granule_rows_admit_v<N, T>;
...
[[nodiscard]] ADM_ALWAYS_INLINE V granule_stage_twiddle(V u) {
    ...
    if constexpr (w.s == 0 && w.c == 1) {
        ...
        static constexpr auto pool =
            granule_pair_pool<T, V::size>(static_cast<T>(-w.c), static_cast<T>(w.c));
        ...
        static constexpr auto pool = granule_pool_of<T, V::size>(w, T(0));
        return xsimd::fma(V(static_cast<T>(w.c)), u,
                          xsimd::mul(V::load_unaligned(pool.data()), granule_swp(u)));
    }
}
