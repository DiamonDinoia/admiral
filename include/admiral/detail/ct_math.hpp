...
//   `ct_sincos_*`                     : ADM_CONSTEVAL sin/cos of a rational turn
//                                       fraction.
//   `smallest_radix` / `codelet_radix`: radix to peel off N.
// Split from `codelet.hpp` so the DIF pass headers (butterfly, twiddles) avoid
// re-instantiating `kernel<N>` in every consumer TU (`kernel<N>` lives in
// admiral_codelets).
// ============================================================================
...
// backend asks for more. A double constant caps a transform at 2^-53, and the
// tolerance of `long double` sits 11 bits below 2^-53.
// ----------------------------------------------------------------------------
...
[[nodiscard]] ADM_CONSTEVAL ct_sincos_v<F> ct_sincos_small(F x) {
    ...
    //
    // The loop runs until both tails fall under eps(F), not to a fixed count: a
    // count is calibrated to ONE mantissa width, and `long double` is 64 bits on
    // x86, 113 on aarch64 and 53 on arm64 macOS. `ct_sincos_turns` is the
    // butterflies' only twiddle source, so its fold error is the transform's phase.
    // Terms shrink monotonically over |x| <= pi/4, so the test terminates.
...
// pass the Forward flag straight through and keep num/den unsigned. Every
...
[[nodiscard]] ADM_CONSTEVAL ct_sincos_v<F> ct_sincos_turns(bool conjugate, std::size_t num,
    ...
    // Rotate by base (multiple of pi/4); `inv_sqrt2` = sqrt2/2 (exact).
...
// (direct O(N^2)). The runtime driver routes large primes to Bluestein, so
// `kernel<N>` is instantiated only for smooth N.
// ----------------------------------------------------------------------------
...
//
// The trial divisor steps by 1, not 2, so an EVEN n reaches the (odd, pow2)
// split. Pure powers of two are unaffected: every a dividing 2^m shares a factor
// with 2^m/a, so 2^m still returns {0,0} and stays on `pow2_dif_butterfly`.
...
[[nodiscard]] constexpr std::size_t codelet_radix(std::size_t N) {
    if (N == 60) return 5;
    // N=32: peel r=8 (M=4) over r=4 (M=8). r=8 spills (`peak_live` 26 > 16) but
    // the arithmetic of r=8 wins on wide out-of-order uarchs. N=64: r=8 is
    // register-starved; keep r=4.
    if (N == 32) return 8;
...
// Rader converts a prime `kernel<p>` into a length-(p-1) cyclic convolution, which
// replaces a spilling flat O(p^2) DFT.
// ----------------------------------------------------------------------------
...
// avoid the O(p^2) unroll. 13 is flat because `radix_sym_dft<13>` costs less than
// the two 12-point transforms plus rotations plus permutation gathers of Rader-13,
// and 13 emits less code. 17 is not flat, because the cofactor batch changes shape
// at 2*17 and up.
// The bound is a `kernel<N>` choice only. The bound must not widen
// `is_codelet_supported`: the DIF tape has no thunk for a prime above 11 in a first
// or last pass, and claiming one sends every two-factor size through an unbuildable
// chain.
// `codelet_cost_cyc_f64` and `codelet_cost_cyc_f32` price these leaves. Re-sweep
// both tables whenever this bound moves.
...
// physical sweeps that reload the inputs (see `dif_butterfly_wants_reload`).
...
// A radix-IP butterfly holds 2*IP live batches plus twiddles and addresses, so
// the register file the butterfly can use is a fraction of the register file the ISA
// has. A fraction rather than a fixed headroom keeps the bound positive on a
// narrow-register ISA. Callers pass `poet::vector_register_count()`; at 32 registers
// the bound is 24.
