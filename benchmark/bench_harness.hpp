...
//
// The N-D and r2c compares instantiate the `nd_plan` -> `col_dif_dispatch` chain
// twice over (c2c and r2c, two precisions). In their own TU that compile cost runs
...
// at a 1ms epoch floor and, while noisy (`err` > `kStableMdape`), double the epoch
// budget up to a cap. `min_iters` > 0 forces exact `minEpochIterations` and disables
// adaptation.
...
// or under the line; every reporter labels a run above the line UNSTABLE.
inline constexpr double kStableMdape = 0.05;

// --pass. Times `dif_pass` / `dif_pass_last` directly to ground the per-pass cost
// surface, which is non-monotone in `ido`: vectorized at `ido` >= W and again at
// `ido` == 1 (lane-over-b last pass), scalar in between. `perf_iters` > 0 runs a
// bare loop with no nanobench, so perf stat reads almost-pure kernel PMU.
template<typename T>
...
NbStat nb_measure(const char* name, int reps, long min_iters, Func&& func) {
    ...
    for (int ms = 1; ms <= kMaxMs; ms *= 2) {
        ...
        b.run(name, func);   // re-invoked per retry; do not move from func
...
// Median absolute deviation about the median: an outlier-insensitive spread, in the
// ratio's own units.
...
// blocks carry a measurement-order artifact, so `engine_ab_core` instead:
...
// An engine is two "run once" thunks (`fwd`, `rt`); its maker allocates its state,
// so role-swap swaps allocation order.
...
// is the correct metric. The label differs from the counters-missing case.
...
double engine_ab_core(const char* tag, const std::string& shape_str, const char* prec,
    ...
    auto run_phase = [&](auto&& makeFirst, auto&& makeSecond) -> phase {
        ab_engine first  = makeFirst();    // allocated first (carries the gamma bias)
    ...
    const double mfwd = std::sqrt(mab_f / mba_f);   // (tA/tB), gamma cancelled
...
// against ducc0's N-D c2c and, with -DADM_BENCH_FFTW, FFTW's. Forward and
...
// c2c A/B under the --robust gate: the library's plan vs ducc0 (and FFTW when built),
// with the mandatory identity control. `nthreads` threads both arms; the identity
// control then also validates the threaded harness, which a T>1 verdict needs.
template<typename T>
bool compare_nd_robust(const std::vector<std::size_t>& shape, int rounds, int reps, long inner,
                       int nthreads = 1);

// r2c A/B under the --robust gate: `plan_r2c` vs ducc0 r2c/c2r (and FFTW when built),
// identity-gated.
template<typename T>
...
#ifdef ADM_BENCH_FFTW
// FFTW_MEASURE by default: FFTW_ESTIMATE is an untuned heuristic plan, while a real
...
// FFTW's threads layer initializes once per precision; `fftw*_plan_with_nthreads` is
// per plan. A failed init leaves the serial planner, so the count falls back to 1
// rather than promising threads the library cannot deliver.
template<typename T>
...
public:
    ...
    // General-rank c2c: `fftw_plan_dft` covers any rank in one path; planning cost
    // is paid once here. `nthreads` > 1 needs -DADM_BENCH_THREADS.
...
#endif
        // The inverse plans `out_` -> `in_` so `roundtrip()` chains the two with no
        // copy; the paired library arm chains in place for free.
        if constexpr (std::is_same_v<T, double>) {
    ...
    // FFTW's c2c is unnormalized, so the scale is work FFTW genuinely owes.
