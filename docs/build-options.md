...
| `ADM_CXX_STANDARD` | The standard the library and its public headers compile at: `17`, `20` or `23`. The headers carry one spelling per construct, valid at both, so the two builds generate the same code. They are not link-compatible: `admiral::span` is `std::span` at C++20 and a polyfill at C++17 | `20` |
