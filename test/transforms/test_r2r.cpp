...
TEMPLATE_TEST_CASE("r2r fct replaces the default scale", "[r2r]", float, double) {
    ...
    // fct is an exact output scale: it REPLACES the default, so it scales linearly and
    // fct == the default reproduces the default path bit for bit.
    ...
    // forward's default, and nothing else: at fct == 1 that direction is the type-2
